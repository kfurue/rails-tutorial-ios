...
class Decoders {
    ...
    private static var __once: () = {
        ...
        // Decoder for [Micropost]
        Decoders.addDecoder(clazz: [Micropost].self) { (source: AnyObject, instance: AnyObject?) -> [Micropost] in
            return Decoders.decode(clazz: [Micropost].self, source: source)
        }
        // Decoder for Micropost
        Decoders.addDecoder(clazz: Micropost.self) { (source: AnyObject, instance: AnyObject?) -> Micropost in
            let sourceDictionary = source as! [AnyHashable: Any]
            let result = instance == nil ? Micropost() : instance as! Micropost
            
            result.id = Decoders.decodeOptional(clazz: Int.self, source: sourceDictionary["id"] as AnyObject?)
            result.content = Decoders.decodeOptional(clazz: String.self, source: sourceDictionary["content"] as AnyObject?)
            result.userId = Decoders.decodeOptional(clazz: Int.self, source: sourceDictionary["user_id"] as AnyObject?)
            result.picture = Decoders.decodeOptional(clazz: Picture.self, source: sourceDictionary["picture"] as AnyObject?)
            return result
        }


        // Decoder for [Picture]
        Decoders.addDecoder(clazz: [Picture].self) { (source: AnyObject, instance: AnyObject?) -> [Picture] in
            return Decoders.decode(clazz: [Picture].self, source: source)
        }
        // Decoder for Picture
        Decoders.addDecoder(clazz: Picture.self) { (source: AnyObject, instance: AnyObject?) -> Picture in
            let sourceDictionary = source as! [AnyHashable: Any]
            let result = instance == nil ? Picture() : instance as! Picture
